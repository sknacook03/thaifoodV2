...
if (isset($_POST['update'])) {
  ...
  $check = $conn->prepare("SELECT name FROM drink WHERE name = :name AND id != :id");
  $check->bindParam(':name', $name, PDO::PARAM_STR);
  $check->bindParam(':id', $id, PDO::PARAM_INT);
  $check->execute();
  $count = $check->rowCount();
  if ($count > 0) {
    $_SESSION['error'] = "ชื่อเครื่องดื่มนี้มีอยู่แล้วในระบบ";
      ...
      <div class="page-content d-flex justify-content-center">
        <div class="con-edit">
          ...
          <form class="edit-form" action="edit.php" method="post" enctype="multipart/form-data">
            ...
            if (isset($_GET['id'])) {
              $id = $_GET['id'];
              $stmt = $conn->query("SELECT * FROM drink WHERE id = $id");
              $stmt->execute();
              $data = $stmt->fetch();
            }
            ?>
            ...
            <div class="modal-footer gap-1">
              <a class="btn btn-secondary" href="admin-drink.php">Back</a>
